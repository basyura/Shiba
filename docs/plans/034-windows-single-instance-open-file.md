...
- `src/windows.rs` に Windows 限定の単体テストを追加する。
- 実ウィンドウや Explorer には依存せず、`WM_COPYDATA` の payload 作成と復元を確認する。
- 正しい `WM_COPYDATA` と payload で `PathBuf` が復元されることを確認する。
- 別メッセージ、異なる `dwData`、壊れた payload は無視されることを確認する。
